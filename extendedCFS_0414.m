close all 
clear all 
testn = [21 31 41 51 61 ];
for tt=1:length(testn)
...
%         end
    ...
    zT = [pathnew(1*dim+1)+nstep;0];
    


    if norm(refpath-pathnew) < 0.1 || k == k_lim
...
bar(mstable())
set(gcf, 'position', [0 0 500 200]);
title(['M-stable analysis  (n= ' num2str(nstep) ')'])
...
bound =[];
grow = 1:nstep-1;
bound = [ 0 grow ones(1,tester-nstep)*(nstep -1)];
error(tt,:) = bound(1:tester) - mstable(1:tester);
small2(tt) = min(mstable(nstep:end),[],2);

bound =[];
grow = 1:21-1;
bound = [ 0 grow ones(1,tester-21)*(21 -1)];
error2(tt,:) = bound(1:tester) - mstable(1:tester);
...
figure
plot(error')
legend(['(H= ' num2str(testn(1)-1) ')'],['(H= ' num2str(testn(2)-1) ')'],['(H= ' num2str(testn(3)-1) ')'],['(H= ' num2str(testn(4)-1) ')'],['(H= ' num2str(testn(5)-1) ')'],'location','Northwest')

%legend(['(H= ' num2str(testn(1)-1) ')'],['(H= ' num2str(testn(2)-1) ')'],['(H= ' num2str(testn(3)-1) ')'],'location','Northwest')
title('Difference between simulated M and M_{L,k}');
xlabel('k^{th} action location')
ylabel('Difference')
...
legend(['(H= ' num2str(testn(1)-1) ')'],['(H= ' num2str(testn(2)-1) ')'],['(H= ' num2str(testn(3)-1) ')'],['(H= ' num2str(testn(4)-1) ')'],['(H= ' num2str(testn(5)-1) ')'],'location','Northwest')
title('Difference between simulated M and M_{L,k} for H=20');
xlabel('k^{th} action location')
ylabel('Difference')
%%
% for i = 2:length(testn)+3
% saveas(figure(i),[num2str(i) '.eps'])
% end
